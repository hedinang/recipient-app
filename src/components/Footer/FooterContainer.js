...
function FooterContainer(props) {
  ...
  const { delivery, profile } = deliveryStore;

  const client = get(props, 'client') || get(delivery, 'client');
  const settings = get(props, 'settings') || get(deliveryStore, 'settings');
  ...
  if (typeof profile?.props === 'object') dataProps = { ...dataProps, ...profile.props };

  return <Footer {...dataProps} styles={styles} />;
}
