...
function CustomProfile(props) {
  ...
  return (
    <Container maxWidth={false} disableGutters className={classes.container}>
          ...
          <Container className={classes.bodyContainer}>
            ...
            <Box mb={4}>
              <Grid container spacing={mediaQuery.isMobile ? 0 : 2} className={classes.eventsAndMap}>
                <Map mediaQuery={mediaQuery} classes={classes} milestone={milestone} deliveryStore={deliveryStore} delivery={delivery} settings={settings} driverEvent={driverEvent} editedDeliveryData={editedDeliveryData} coordinates={coordinates} podStore={podStore}/>
                <Event classes={classes} delivery={delivery} progress={progress} translatedEvents={translatedEvents} podStore={podStore} history={history}/>
              </Grid>
            </Box>
            {/* <FAQ /> */}
          </Container>
      ...
      <MainDialog store={store} classes={classes} mediaQuery={mediaQuery} step={dialogStep} open={dialogOpened} loading={dialogLoading} />
    </Container>
  );
}
const CustomProfileCompose = compose(
  inject("store"),
  observer
) (CustomProfile);

export default withStyles(styles)(withMediaQuery()(CustomProfileCompose));
